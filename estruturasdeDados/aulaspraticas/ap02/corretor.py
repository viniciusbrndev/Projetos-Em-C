...
class Corrector(object):
    ...
    def __run_valgrind(self, test_case):
        print('  Executando Valgrind...')
        print('  ---------------------')

        cmd = f'valgrind --leak-check=full -s ./exe < {test_case}'
        p = subprocess.Popen(
            cmd,
            shell=True,
        ...
        try:
            stdout, stderr = p.communicate(timeout=self.__max_time * 5)
        except subprocess.TimeoutExpired:
            p.kill()
            print('\x1b[6;30;41m VALGRIND TIMEOUT \x1b[0m')
            return False

        output = (stdout + stderr).decode('utf8')


        print(output)
        print('  ---------------------')
        if re.search(r'definitely lost:\s*[1-9]', output):
            print('\x1b[6;30;41m MEMORY LEAK DETECTADO \x1b[0m')
            return False

        print('\x1b[6;30;42m SEM LEAKS \x1b[0m')
        return True
    ...
    def run(self):
        ...
        if compilation_response == 'error':
            ...
            if input_cases:
                if not self.__run_valgrind(input_cases[0]):
                    print('\x1b[6;30;43m  Vazamento de memória detectado! Nota zerada.  \x1b[0m')
                    grade = 0.0
